...
# ⚠️ Put your real API key here (better to load from a file or env var in real usage)
...
PROMPT_VERSION = "4"  # or "3" etc., whatever your working version is
...
class TransparentBorderWindow(QWidget):
    ...
    def capture_and_send(self):
        ...
        # Instruction to force JSON and precise coordinate system
        instruction = (
            f"Analyze the image and respond ONLY as a JSON object. "
            f"The image resolution is {img_w}x{img_h} pixels. "
            f"Use a 2D coordinate system where (0,0) is the TOP-LEFT corner, "
            f"x increases to the RIGHT and y increases DOWN. "
            f"Return the field 'Answer location' as [x, y] in THIS pixel system, "
            f"pointing roughly at the CENTER of the correct answer. "
            f"The JSON must have exactly these keys: "
            f"'Question read', 'Answer selected', 'Answer location'."
        )
        ...
        print(f"Model returned image coords: ({x_img}, {y_img})")
